export interface SystemParams {
  ...
  klineTestMinutes?: number;
  klineSetHours?: number;
...
export function calculatePlan(type: string, params: SystemParams): Plan {
  if (type === 'pivot') {
    ...
    const sectionCLen = arm - sectionCStart;
    ...
    const pivotSections: PivotSection[] = [
      { name: 'Section A (Inner)', from: 0, to: sectionAEnd, buckets: 0, spacing: 0, sectionLength: sectionAEnd, isExcluded: true },
      { name: 'Section B (Mid spans)', from: sectionBStart, to: sectionBEnd, buckets: bBuckets, spacing: bSpacing, sectionLength: sectionBLen },
      { name: 'Section C (Outer spans)', from: sectionCStart, to: arm, buckets: cBuckets, spacing: cSpacing, sectionLength: sectionCLen },
      ...(hasGun ? [{ name: 'End Gun', from: arm, to: gunEnd, buckets: gunBuckets, spacing: gunSpacing, sectionLength: (gunBuckets - 1) * gunSpacing, isGun: true }] : []),
    ];
    ...
    return { bucketCount: totalBuckets, spacing: bSpacing, pattern, startOffset: sectionBStart, armLength: arm, numSpans: params.spans, pivotSections };
  }
  ...
  return { bucketCount: count, spacing: Number(spacing.toFixed(1)), pattern };
}
